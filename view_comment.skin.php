...
<?php
// [수정 3] 코멘트가 하나 이상 있을 때만 ul을 출력 — 빈 흰 박스 노출 방지
if (count($list) > 0) :
?>
<!-- 코멘트 리스트 -->
<ul>
...
?>
    <li>
        ...
        <div class="qna-comment-content theme-box">
            ...
            $str = preg_replace("/\[\<a\s*href\=\"(http|https|ftp)\:\/\/([^[:space:]]+)\.(gif|png|jpg|jpeg|bmp)\"\s*[^\>]*\>[^\s]*\<\/a\>\]/i", "<img src='$1://$2.$3' id='target_resize_image[]' onclick='im_layer(this);' />", $str);
            echo $str;
            ?>
        </div>
        <p>
...
<div class="ui-write-area" id="comment_write<?=$wr_id?>" style="display:none;">
    ...
    <form name="fviewcomment<?=$wr_id?>" method="post" action="./write_comment_update.php" autocomplete="off">
        ...
        <?php if ($comment_min || $comment_max) { ?>onkeyup="check_byte('wr_content', 'char_count');"<?php } ?> style='width:100%; word-break:break-all;' class='tx'><?=$list[$i]['wr_content']?></textarea>
